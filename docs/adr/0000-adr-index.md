...
| 0090 | Aggregates reach observations only through named selectables | `price_observations` keeps superseded rows by design, so an aggregate missing the status predicate returns a figure inflated by however much reprocessing has happened, and a null `product_id` groups unrelated goods together; both exclusions move into `countable_observations()` and `unresolved_observations()`, enforced by the `branch-scope` gate | Accepted |
